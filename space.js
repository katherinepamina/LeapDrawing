...
Leap.loop(function(frame) {
    ...
    frame.hands.forEach(function(hand, index) {
      ...
      for(var i = 0; i < 25; i++){
        var asteroid = (asteroids[i] || (asteroids[i] = new Asteroid()));
        var pos = hand.screenPosition();
        if(Math.abs(asteroid.position[0]-pos[0]) < asteroid.siz/2 && Math.abs(asteroid.position[1]-pos[1]) < asteroid.siz/2){
          ...
          for(var j = 0; j < shots.length; j++){
            if(Math.abs(asteroid.position[0]-shots[j].position[0]) < asteroid.siz/2 && Math.abs(asteroid.position[1]-shots[j].position[1]) < asteroid.siz/2)
            {
              ...
              asteroid.position[1] = Math.random()*10;
              asteroid.siz = 10;
            }
          }
        }
          asteroid.position[0] += 5;
...
var SpaceShip = function() {
  ...
  img.style.height = '10%';
  img.onload = function () {
    ship.setTransform([window.innerWidth/2,window.innerHeight/2], 0);
    document.body.appendChild(img);
  }

  ship.setTransform = function(position, rotation) {
...
var Asteroid = function() {
  ...
  img.onload = function () {
    asteroid.setTransform([window.innerWidth/2,window.innerHeight/2], 0);
    document.body.appendChild(img);
  }

  asteroid.setTransform = function(position, siz) {
